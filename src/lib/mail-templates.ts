...
function button(href: string, label: string): string {
  return `<p style="margin:26px 0"><a href="${href}" style="display:inline-block;background:#e2b340;color:#000000;font:700 14px ui-monospace,Menlo,monospace;letter-spacing:.04em;text-transform:uppercase;text-decoration:none;padding:14px 22px">${label}</a></p>`;
}

export function seatLinkMail(to: string, name: string, link: string): EmailMessage {
  const subject = "Your spot on flexwall.lol";
  const text = `Here is the link to your spot on flexwall.lol (${name}):\n\n${link}\n\nIt works for 30 minutes, then you can ask for a new one from flexwall.lol/me.\nIf you did not ask for this, ignore this email.`;
  const html = shell(
    `<h1 style="font:800 28px -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;margin:0 0 12px">Your spot, <span style="color:#e2b340">${esc(name)}</span>.</h1>
<p style="margin:0;color:#8b98a5">Open the link below to see your rank and your payments.</p>
${button(link, "Open my spot")}
...
export function welcomeMail(
  ...
  const subject = `You're #${opts.rank} on flexwall.lol`;
  const text = `${opts.name}: #${opts.rank} of ${opts.total} for ${amount}.\nGood luck keeping it.\n\nYour spot (rank, payments, who is above you):\n${opts.link}\n\nShare card:\n${opts.shareUrl}\n\nPost it on X:\n${opts.xUrl}\n\nThis link works for 7 days. Later, get a fresh one from flexwall.lol/me with this email.\nNo refunds.`;
  const html = shell(
    `<h1 style="font:800 28px -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;margin:0 0 12px">You're <span style="color:#e2b340">on the wall.</span></h1>
<p style="margin:0 0 6px;font-size:18px"><b>${esc(opts.name)}</b> · ${amount} · rank <b style="color:#f2c75c">#${opts.rank}</b> of ${opts.total}</p>
<p style="margin:0;color:#8b98a5">Good luck keeping it.</p>
${button(opts.link, "Open my spot")}
...
export function passedMail(
  ...
  const html = shell(
    `<h1 style="font:800 28px -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;margin:0 0 12px">You got <span style="color:#e5484d">outbid.</span></h1>
<p style="margin:0 0 6px;color:#8b98a5"><b style="color:#e7e9ea">${esc(opts.byName)}</b> paid ${formatUSD(opts.byAmountUSD)} and took your spot. Someone wanted it more.</p>
<p style="margin:0;color:#8b98a5">You are now <b style="color:#f2c75c">#${opts.newRank}</b> of ${opts.total}.</p>
${button(opts.link, "Take it back · +" + reclaim)}
<p style="margin:0;color:#8b98a5;font-size:14px">Top-ups have no minimum. The wall keeps the whole history either way.</p>`,
    "You get this email because your spot on flexwall.lol lost a rank. No refunds.",
  );
  return { to, subject, html, text };
}
