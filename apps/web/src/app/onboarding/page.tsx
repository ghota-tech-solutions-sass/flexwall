...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<Partial<Record<(typeof ONBOARDING_PARAMS)[keyof typeof ONBOARDING_PARAMS], string>>> }) {
  ...
  if (user.handle) redirect(plan ? ROUTES.pricingForPlan(plan) : ROUTES.edit);
  // What they typed before signing in. It is still confirmed here: a handle can't be changed afterwards.
  const wanted = params[ONBOARDING_PARAMS.handle];
  return (
    <div className="page">
      <TopBar signedIn />
      <main className="auth">
        ...
        <HandleForm suggested={wanted} plan={plan} />
      </main>
    </div>
  );
}
