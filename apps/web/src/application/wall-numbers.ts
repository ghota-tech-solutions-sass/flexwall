import { showsRange, type NumberValue } from "@flexwall/sdk";
...
export interface WallNumber {
  ...
  verified: boolean;
  /** Printed as a range ("$1M+") wherever it leaves the tile: the metric is sensitive or the tile asks for it. */
  range: boolean;
...
export function wallNumbers(tiles: readonly Tile[], states: Record<string, TileState>, catalog: Catalog): WallNumber[] {
  return tiles.flatMap((tile) => {
    ...
    return Object.entries(tile.inputs).flatMap(([key, binding]) => {
      ...
      const range = showsRange(tile.options.display, input.source?.sensitive);
      return [{ tile, binding, value: input.value, verified: Boolean(input.source?.verified), range, label }];
    });
  });
}
