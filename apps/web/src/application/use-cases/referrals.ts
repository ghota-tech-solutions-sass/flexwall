...
import type { AppLinks, Clock, ReferralRepository, UserRepository } from "../ports";
...
export class GetReferralProgram {
  constructor(private readonly deps: { users: UserRepository; referrals: ReferralRepository; clock: Clock; links: AppLinks }) {}
  ...
  async execute(input: { userId: string }): Promise<ReferralProgram | null> {
    ...
    return {
      link: user.handle ? this.deps.links.referral(user.handle) : null,
