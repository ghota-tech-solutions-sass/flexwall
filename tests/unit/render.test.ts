...
describe("render", () => {
  test("every theme renders a PNG at device resolution", async () => {
    for (const theme of THEME_IDS) {
      ...
      const buf = await renderWallpaper(config, await resolveWall(config, new Date(), sampleGithub), { watermark: true }).arrayBuffer();
  ...
  test("every device keeps its aspect ratio, and width overrides scale it", async () => {
    for (const device of DEVICE_IDS) {
      ...
      const data = await resolveWall(config, new Date(), sampleGithub);
